...
function makesprinklers()
	if (game:GetService("Workspace").FlowerZones[kometa.vars.field].Position - api.humanoidrootpart().Position).magnitude < temptable.magnitude then
		...
		for Index = 1, Amount do
			...
			if Amount == 1 then
				api.tweenDelay(3, CFrame.new(FieldSelected.Position))
				task.wait(.1)
				api.humanoid().Jump = true
			elseif Amount ~= 2 then
				api.tweenDelay(3, CFrame.new(FieldBounds[Index]))
				task.wait(.1)
				api.humanoid().Jump = true
			else
				if Index == 1 then
					api.tweenDelay(3, CFrame.new(FieldBounds[1]))
					task.wait(.1)
					api.humanoid().Jump = true
				else
					api.tweenDelay(3, CFrame.new(FieldBounds[3]))
					task.wait(.1)
					api.humanoid().Jump = true
				end
			end
	...
	for i,v in pairs(game:GetService("Workspace").MonsterSpawners:GetChildren()) do
		if v:FindFirstChild("Territory") then
			if v.Name ~= "Commando Chick" and v.Name ~= "CoconutCrab" and v.Name ~= "StumpSnail" and v.Name ~= "TunnelBear" --[[and v.Name ~= "King Beetle Cave"]] and not v.Name:match("CaveMonster") and not v:FindFirstChild("TimerLabel", true).Visible then
				...
				repeat avoidmob() task.wait(1.2) api.tween(3, monsterpart.CFrame + Vector3.new(0, 30, 0)) until v:FindFirstChild("TimerLabel", true).Visible or api.humanoid().Health == 0
...
function getfuzzy(v)
	...
	repeat
		task.wait()
		api.walkTo(Vector3.new(FuzzyPlane.Position.X, FuzzyPlane.Position.Y+3, FuzzyPlane.Position.Z))
...
function UseDispensers()	
	if kometa.toggles.honeystorm and CheckToyCooldown('Honeystorm') then disableall()
	...
	if kometa.toggles.autoboosters then
		if kometa.dispensesettings.white and CheckToyCooldown('Field Booster') then disableall()
...
task.spawn(function() while task.wait(5) do
		if temptable.converting == false then
			--disableall()
			UseDispensers()
			--enableall()
		end
...
if _G.autoload ~= nil then if isfile("kometa/BSS_".._G.autoload..".json") then kometa = game:service'HttpService':JSONDecode(readfile("kometa/BSS_".._G.autoload..".json")) end end
