...
local kometa = {
	...
	local distance = Vector3.new(pos1.X, pos1.Y, pos1.Z) - Vector3.new(pos2.X, pos2.Y, pos2.Z)
	...
	["tween"] = function(time, pos) -- tween to position by (time) + (frame)
		...
		local tween = game:GetService("TweenService"):Create(plr, TweenInfo.new(temp_time, Enum.EasingStyle.Linear), {CFrame = CFrame.new(pos.X, pos.Y, pos.Z)})
	...
	["tweenDelay"] = function(time,pos)
		...
		game:GetService("TweenService"):Create(plr, TweenInfo.new(time, Enum.EasingStyle.Linear), {CFrame = CFrame.new(pos.X, pos.Y --[[+5]], pos.Z)}):Play()
		createplatform(time)
	end,
